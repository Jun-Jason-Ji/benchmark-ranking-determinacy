...
- 对照本项目测得的策略种子噪声。两种噪声模型都在论文中报告，不可混用：按整个种子集之间的 sd 计算（`analyze_seed_noise.py`，茄子 64 构型），单种子 95% 半宽 ±0.132、3 个种子 ±0.076；按 Eq. (eq:var) 汇总每构型运行方差计算（论文各表所用），3 个种子的逐对半宽为 0.075–0.088。上面每个格子与公布值的差都落在这两者之内。
