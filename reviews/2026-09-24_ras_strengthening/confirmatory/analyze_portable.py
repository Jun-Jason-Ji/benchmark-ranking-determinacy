  ...
  for batch in range(4):
    ...
    for mode in MODES:
      for setting in SETTINGS:
        p=HERE/'raw_v2'/f'{task}_{mode}_{setting}_batch{batch}.json'
        rec=json.loads(p.read_text())
        assert rec['steps']==100 and rec['num_scenes']==256
        assert rec['plan_sha256']==frozen['files']['PLAN.md']
        assert rec['evaluator_sha256']==frozen['files']['run_confirmatory_v2.py']
...
report=dict(status='complete',primary_rollouts=20480,steps_per_rollout=100,
            ...
            raw_sha256=raw_hashes,freeze_sha256=sha(HERE/'FROZEN_RUNTIME_AMENDMENT_V2.json'))
