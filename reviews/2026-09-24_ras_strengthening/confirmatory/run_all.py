...
for task in ['PickCube-v1','PushCube-v1']:
    for mode in ['pd_joint_delta_pos','pd_ee_delta_pos']:
        remaining=1500-(time.time()-started)
        if remaining<=0:raise TimeoutError('Fixed execution budget exhausted')
        name=f'{task}_{mode}'
        t=time.time()
        with (HERE/(name+'.log')).open('w',encoding='utf-8') as stream:
            proc=subprocess.run([sys.executable,str(HERE/'run_confirmatory.py'),'--task',task,'--mode',mode],
        ...
        jobs.append(item)
        (HERE/'PROGRESS.json').write_text(json.dumps(dict(jobs=jobs,elapsed=time.time()-started),indent=2),encoding='utf-8')
