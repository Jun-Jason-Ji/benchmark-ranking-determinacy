"""Analyse V3 controlled-initialization direction repeatability, or fail closed.
...
def main():
    ...
    require(frozen.get('protocol_version')=='V3-controlled-initialization','Wrong protocol version')
    require(frozen.get('reset_validation_passed') is True,'Reset validation did not pass before freeze')
            ...
            for condition in CONDITIONS:
                ...
                for event in audit:
                    ...
                    if event['kind']=='environment_reset':
                        ...
                        require(event.get('nominal_settling_completed_before_condition')is True and event.get('physics_steps_after_condition')==0,'Condition applied before canonical initialization')
                        require(event.get('active_controller_references_new_scene')is True,'Controller references stale scene')
                        for key,factor,nominal_key in [('stiffness',ss,'arm_stiffness'),('damping',ds,'arm_damping'),('force_limit',1.,'arm_force_limit')]:
                            expected=np.asarray(expected_nominal[nominal_key])*factor
                            require(np.allclose(event['built_controller'][key],expected,rtol=2e-6,atol=1e-5),'Reset controller configuration mismatch')
                            require(np.allclose(event['actual_joints'][key],expected,rtol=2e-6,atol=1e-5),'Actual physical drive mismatch')
                ...
                closures=[e for e in audit if e['kind']=='episode_closed']
                require({e['episode_id']for e in closures}==set(range(24)),'Missing fresh-environment lifecycle completion')
                require(all(e.get('closed_scene_and_agent_cleared')is True and e.get('active_parameters_unchanged')is True for e in closures),'Episode did not preserve parameters or close completely')
    ...
    result=dict(status='complete',raw_rollouts=768,n_independent_blocks_assumed=8,configurations_per_block=24,
        ...
        scope='V3 controlled initialization after exclusion of a technical attempt with unmatched initial states. Independent identically sampled policy-seed blocks are assumed; within-block dependence is unrestricted. Fixed finite scene population, fixed pipelines/build, final-success horizon 60.',
        exclusions='Not a test of E[D]=0; not a six-setting envelope confirmation, real-world or cross-engine validation. This tests the direction under a repaired initialization protocol, not causal isolation of the historical protocol contrast.',
    ...
    lines=['# V3 prospective spoon repeatability with controlled initialization','',
