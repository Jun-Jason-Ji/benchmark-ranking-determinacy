"""V3 isolated scene initialization; one fresh environment per episode.

This file executes only within a frozen, reset-validated V3 study. It leaves
...
def main():
    ...
    ap.add_argument('--condition',required=True,choices=['nominal','fitted'])
    ap.add_argument('--policy-seed-base',type=int,required=True)
    ap.add_argument('--output-dir',required=True)
    args=ap.parse_args()
    frozen=json.loads((HERE/'FROZEN_PLAN.json').read_text())
    check(frozen.get('protocol_version')=='V3-controlled-initialization'and frozen.get('reset_validation_passed')is True,'V3 is not validated/frozen')
    ...
    from fresh_reset import fresh_reset_episode,joint_properties,controller_properties,contact_properties
    ...
    source_paths=[Path(__file__),RESET/'fresh_reset.py',ROOT/'scripts/controller_sweep.py',ROOT/'scripts/octo_policy_server.py',ROOT/'scripts/ms3_windows_compat.py']
    ...
    metadata=dict(started_utc=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),
       protocol_version='V3-controlled-initialization',
       ...
       nominal=sweep.NOMINAL,initialization='fresh environment; nominal settling; then arm condition applied without physics advance',
    ...
    for ep in range(24):
        ...
        with fresh_reset_episode(env_id,ep,condition,render_backend)as(env,obs,initial):
            ...
            delay=0 if condition=='nominal'else 1
            ...
            rec=dict(policy=args.policy_name,env_id=env_id,condition=condition,episode_id=ep,policy_seed=seed,
                     ...
                     episode_seconds=time.perf_counter()-started,protocol_version='V3-controlled-initialization')
            effective=dict(arm_stiffness=initial['built_controller']['stiffness'],arm_damping=initial['built_controller']['damping'],
                ...
                actual_joints=initial['actual_joints'],initialization_protocol='fresh_nominal_settled_then_condition')
