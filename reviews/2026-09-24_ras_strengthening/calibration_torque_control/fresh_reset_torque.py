...
def fresh_reset_episode(env_id,episode_id,condition,render_backend):
    ...
    if condition not in ['nominal','forcehalf']:raise ValueError(condition)
    ...
    try:
        ...
        arm.config.stiffness=list(sweep.NOMINAL['arm_stiffness'])
        arm.config.damping=list(sweep.NOMINAL['arm_damping'])
        ...
        assert before==after,'Assigning torque limit changed canonical state'
        ...
        audit.update(dict(episode_id=int(episode_id),condition=condition,
            ...
            delay_steps=0,force_limit_scale=force_scale,
