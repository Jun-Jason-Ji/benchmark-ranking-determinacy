...
which it uses:
  ...
  --demos          restrict to the demonstrations present in every condition (always done) and
                   optionally to a count, for comparison with the older 40-demonstration runs.
...
def main():
    ...
    print(f"search domain: {len(conds)} grid points; paired on {len(ids)} common demonstrations")
